...
if __name__ == '__main__':
    ...
    if args.model == 'cnn':
        if args.dataset == 'femnist':
            ...
            net_glob = ModelFlexFL(args.model, args.out_dim, args.num_classes)
        elif 'cifar' in args.dataset:
            net_glob = CNNCifar(args)
    elif 'resnet' in args.model:
        if args.model == 'resnet_smart':
            # SmartFL ResNet with improved BasicBlock structure
            net_glob = ResNet18_SmartFL(num_channels=args.num_channels, num_classes=args.num_classes)
        else:
            # FlexFL original ResNet (default)
