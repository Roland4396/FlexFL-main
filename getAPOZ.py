...
def hook(args, net, dataset_test, iter, run=None):
    ...
    if args.model == "vgg":
        ...
        # SmartFL ResNet110: 109个ReLU，分布在5个stage
        # [初始conv:1] + [layer1:9blocks×2=18] + [layer2:9blocks×2=18] + [layer3:18blocks×2=36] + [layer4:18blocks×2=36]
...
def getNet(args, rate):
    if args.model == "vgg":
        ...
        net = ResNet18_SmartFL(num_classes=args.num_classes, track_running_stats=False, num_channels=args.num_channels,
                               rate=rate).to(args.device)
    elif args.model == "mobilenet":
...
def calculateScale(args, net_glob, APOZ):
    ...
    # Generate 4 models with ratios: 1/8, 1/4, 1/2, 1
    ans = [torch.ones(len(APOZ))]  # Model 3: 1.0 (100%)
    ans.insert(0, get_net_scale(args, 0.5, scale_rate))    # Model 2: 0.5 (50%)
    ans.insert(0, get_net_scale(args, 0.25, scale_rate))   # Model 1: 0.25 (25%)
    ans.insert(0, get_net_scale(args, 0.125, scale_rate))  # Model 0: 0.125 (12.5%)
