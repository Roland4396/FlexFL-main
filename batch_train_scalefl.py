...
# ScaleFL特定参数
# Scale值计算：sqrt(参数量比例) -> sqrt(1/8), sqrt(1/4), sqrt(1/2), sqrt(1)
WIDTH_RATIOS = [0.354, 0.5, 0.707, 1.0]  # 对应 1/8, 1/4, 1/2, 1 参数量
CLIENT_RATIO = "1:1:1:1"  # 4个等级的客户端均匀分布
GAMMA = 0.05  # ScaleFL的gamma参数
EPOCHS = 1000

# 输出目录
OUTPUT_BASE = "outputs_scalefl"
...
def run_experiment(model, dataset, data_config, exp_id, total_exp):
    ...
    # 构建实验名称
    exp_name = f"scalefl_{model}_{dataset}_{data_config['name']}"
    log_file = os.path.join(OUTPUT_BASE, f"{exp_name}.log")

    print("\n" + "="*80)
    print(f"[{exp_id}/{total_exp}] 开始实验: {exp_name}")
    print(f"开始时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Scale配置: {WIDTH_RATIOS} (对应参数量: 1/8, 1/4, 1/2, 1)")
    print("="*80)
...
def main():
    """主函数"""
    print("\n" + "="*80)
    print("ScaleFL 批量训练开始")
    print(f"时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Scale配置: {WIDTH_RATIOS}")
    print(f"对应参数量比例: 1/8, 1/4, 1/2, 1")
